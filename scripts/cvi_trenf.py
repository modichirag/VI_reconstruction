...
TRENF = flows.TRENF
CTRENF = flows.Conditional_TRENF
...
class VItrenf(tf.keras.Model):
    ...
    def __init__(self, nc, nlayers=3, name=None):
        ...
        self.trenf = CTRENF(nc, condition=tf.expand_dims(data/data.max(), -1), nlayers=nlayers, fitmean=True, fitnoise=True)
...
def optimizetrenf(model, data, opt,  Rsm, niter=100, callback=callback, citer=50, nsamples=1, saveiter=100, repvi=False):
    ...
    for epoch in range(niter+1):
        ...
        if epoch%citer == 0: 
            ...
            if epoch%saveiter == 0: 
                model.save_weights(spath + '/weights/iter%02d'%(epoch//saveiter))
                print('Weights saved')
            start = time.time()
    return losses 
...
for i in vitrenf.variables: print(i.name)
...
for R in RR:
    ...
    losses = optimizetrenf(vitrenf, tf.constant(dataR), opt, tf.constant(R), 5001, callback, citer=100)
