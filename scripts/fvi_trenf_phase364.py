...
trenfpath = "/mnt/ceph/users/cmodi/galference/dm_fvi/L1000_N0128_ZA-affine-1layer/weights/iter0004"
...
def optimizetrenf(model, data, opt,  Rsm, niter=100, callback=callback, citer=50, nsamples=1, saveiter=20, sample=None, batch_size=int(12//nchains), stepsize=0.01, samples=[]):
...
samplepath = '/mnt/ceph/users/cmodi/galference/dm_hmc/L0200_N0064_ZA-kwts4-fourier-corr/'
...
for i in range(prentrain+1):
    ...
    for itrain in range(min(ntrain, int(ntrain*(2*i/prentrain)))):
        ...
        #batch = tf.concat(allsamples[np.random.choice(allsamples.shape[0], size=nsamples)], axis=0) 
        idx = np.random.choice(i + 1, size=nsamples)
        idx = [-1] + list(idx)
