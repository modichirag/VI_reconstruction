...
parser.add_argument('--order', type=int, default=2, help="ZA or LPT")


vipath = '/mnt/ceph/users/cmodi/galference/dm_fvi/L0200_N0032_LPT-burnin/weights/iter0050'
...
if order == 2: fpath = '/mnt/ceph/users/cmodi/galference/cosmo_hmc/L%04d_N%04d_LPT'%(bs, nc)
...
def unnormalized_log_prob(params,  whitec):
    with tf.GradientTape() as tape:
        ...
        logprob = tfd.Uniform(0.18, 0.33).log_prob(params[0]) + tfd.Uniform(0.65, 0.95).log_prob(params[1])
...
################################################################################
#$3whitec = flowpm.utils.r2c3d(tf.constant(zic, dtype=tf.float32)* nc**1.5)
#$3
#$3py_log_prob = lambda x, y: unnormalized_log_prob(tf.constant(x, dtype=tf.float32), y).numpy().astype(np.float32)
#$3py_grad_log_prob = lambda x, y: grad_log_prob(tf.constant(x, dtype=tf.float32), y).numpy().astype(np.float32)
#$3hmckernel = PyHMC(py_log_prob, py_grad_log_prob, returnV=True)
#$3
#$3params0 = tf.Variable(np.array([0.22, 0.85, 0.6]).astype(np.float32)*np.random.uniform(0.9, 1.1))
#$3q, _, acc, energy, _ = hmckernel.hmc_step(params0.numpy(), 5, 0.01, [whitec])
#$3
...
samples, pyacc = [], []
q = params0.numpy()
vicheck = trenfmodel.VItrenf(nc=32, nlayers=3, evolve=evolvedata)
print(vicheck)
vicheck.load_weights(vipath)

py_log_prob = lambda x, y: unnormalized_log_prob(tf.constant(x, dtype=tf.float32), y).numpy().astype(np.float32)
py_grad_log_prob = lambda x, y: grad_log_prob(tf.constant(x, dtype=tf.float32), y).numpy().astype(np.float32)
...
for i in range(args.nsamples):
    ...
    z = vicheck.sample(1)
    whitec = flowpm.utils.r2c3d(z* nc**1.5)
    
    lpsteps = np.random.randint(lpsteps1, lpsteps2, 1)[0]
    q, _, acc, energy, _ = hmckernel.hmc_step(q.copy(), lpsteps, stepsize, [whitec])
