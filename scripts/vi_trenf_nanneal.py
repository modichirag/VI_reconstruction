...
class VItrenf(tf.keras.Model):
    ...
    def __init__(self, nc, nlayers=3, name=None):
        ...
        self.trenf = flows.TRENF_affine(nc, nlayers=nlayers, fitmean=True, fitnoise=True, meanfield=True)
...
def likelihood(data, z, noiselevel):
    final_field = pmz(z)
    logprob = tfd.Normal(final_field, scale=dnoise*noiselevel).log_prob(data)
    return tf.reduce_sum(logprob, axis=[-3, -2, -1])


    
@tf.function
def bbvi_trenf(model, likelihood, data, noiselevel, nsamples = tf.constant(1)):   
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        logl = tf.reduce_mean(likelihood(data, sample, noiselevel))
...
@tf.function
def bbvi_trenf_rp(model, likelihood, data, noiselevel, nsamples = tf.constant(1)):   
...
def optimizetrenf(model, data, opt, niter=100, callback=callback, citer=50, nsamples=1, saveiter=100, repvi=args.repvi, maxnoise=20.):
    ...
    for epoch in range(niter+1):
        noiselevel = tf.constant(max(1., maxnoise*(1 - epoch/(niter/2))))
        print(epoch, noiselevel)
        if epoch%100 == 0: print(epoch)
        if repvi : loss, grad = bbvi_trenf_rp(model, likelihood, data, noiselevel, nsamples=tf.constant(nsamples))
        else: loss, grad = bbvi_trenf(model, likelihood, data, noiselevel, nsamples=tf.constant(nsamples))
...
losses = optimizetrenf(vitrenf, tf.constant(data), opt, 10001, callback, citer=100)
